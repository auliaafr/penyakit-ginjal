...
import numpy as np
from web_functions import predict  # Pastikan Anda memiliki fungsi predict di web_functions

ginjal_model = pickle.load(open('model_dtc.pkl', 'rb'))
...
def app(df, x, y):
    ...
    with st.form("prediction_form"):
        ...
        if submitted:
            # Konversi input dari teks menjadi float
            try:
                features = [
                    ...
                    1 if nafsu_makan == "Baik" else 0,
                    1 if pedal_edema == "Ya" else 0,
                    1 if anemia == "Ya" else 0
                ]
            except ValueError:
                st.error("Pastikan semua input numerik valid dan tidak kosong.")
                return
